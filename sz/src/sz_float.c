			...
			if(confparams_cpr->errorBoundMode>=PW_REL)
			{
				...
				if(confparams_cpr->szMode == SZ_TEMPORAL_COMPRESSION)
...
#endif				
					{
						...
						{
#endif							
							SZ_compress_args_float_NoCkRngeNoGzip_1D(cmprType, &tmpByteData, oriData, r1, realPrecision, &tmpOutSize, valueRangeSize, medianValue); // jwang
...
unsigned char * SZ_compress_float_2D_MDQ_nonblocked_with_blocked_regression(float *oriData, size_t r1, size_t r2, float realPrecision, size_t * comp_size){
	...
	float rel_param_err = 0.15/3; // 0.15/3
	...
	// use_mean section deleted;
	{
		...
		for(size_t i=0; i<num_x; i++){
			...
			for(size_t j=0; j<num_y; j++){
				...
				//	float _err_sz = 0.0, _err_reg = 0.0;
				//	// [1, 1] [3, 3] [5, 5] [7, 7] [9, 9]
				//	// [1, 9] [3, 7]		[7, 3] [9, 1]
				//	int bmi = 0;
